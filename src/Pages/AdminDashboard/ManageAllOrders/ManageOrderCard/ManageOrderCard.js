...
const ManageOrderCard = ({ detail }) => {
  ...
  const handleDeleteProduct = (id) => {
    ...
    if (proceed) {
      const url = `https://salty-dawn-56606.herokuapp.com/allorder/${id}`;
      ...
      <Card className="shadow-lg mb-5">
        ...
        <Card.Body>
          <div className="d-flex justify-content-between">
            ...
            <h4 className="text-warning fw-bolder">${detail?.order?.price}</h4>
          </div>

          <p className="text-cyan">{detail?.order?.description}</p>

          {
            detail?.status === 'shipped' ? (<h6 className="text-warning fw-bolder">
              {" "}
        ...
        <Card.Footer className="d-flex justify-content-between">
          ...
          <Button
            ...
            className="btn-warning shadow-none py-2 px-1 btn-sm fw-semi-bold"
          >
            Proceed Shipping
          </Button>
        </Card.Footer>
