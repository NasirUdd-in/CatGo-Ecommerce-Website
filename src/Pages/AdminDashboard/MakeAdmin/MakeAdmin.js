...
const MakeAdmin = () => {
  ...
  useEffect(() => {
    fetch("https://salty-dawn-56606.herokuapp.com/users")
      .then((res) => res.json())
      .then((data) => setMakeAdmin(data));
  }, [status]);

  const handleUpdate = (id) => {
    const url = `https://salty-dawn-56606.herokuapp.com/users/${id}`;
    fetch(url, {
      method: "PUT",
      headers: {
  ...
  return (
    <Container className="mb-5 mt-5" style={{ minHeight: "100vh" }}>
      <div className="col-12 col-md-8 mx-auto">
        <h3 className="text-warning text-center mt-5 mb-3 text-decoration-underline">
          {" "}
          All Users List
        </h3>
      </div>
      <div>
        <h3 className="text-center my-3">
          Catgo Has Got Total {makeadmin.length} Users
        </h3>
      </div>
      <Row>
            ...
            {makeadmin.map((user) => (
              <tbody className="text-center" key={user._id}>
                <tr>
                  ...
                  <td>
                    {user?.role === "admin" ? (
                      <h6 className="text-warning fw-bolder">
                        {user?.role} {""}
                      </h6>
                    ) : (
                  ...
                  <td>
                    <Button
                      ...
                      className="btn-warning fw-semi-bold shadow-none"
                    >
                      Make Admin
                    </Button>
                  </td>
